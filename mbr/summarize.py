...
3. `aggregate`: two short sentences, 45 words at most in total, shown on a small card, about the 2-3 biggest themes. Every count must equal the number of reports you listed for that theme. Numbers count reports only, never replies or quoted tweets. When the replies add something, such as many people agreeing, say so in words (e.g. "one report, with many replies agreeing"). With {n} reports in total, no count can exceed {n}. When a theme rests on one report, say "one report" rather than implying a trend. Example: "4 reports of pushing back when the user is wrong, 3 of taking actions it wasn't allowed to. One report also finds it unusually funny." Do not list examples in parentheses.
